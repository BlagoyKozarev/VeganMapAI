...
- **Production Sofia Restaurant Debug (Feb 4, 2025)**: Investigated missing Sofia restaurants in production deployment. Database contains 387 Sofia restaurants with valid vegan scores. Issue identified: `/api/restaurants/all-available` endpoint requires authentication (isAuthenticated middleware). Created debug-sofia-restaurants.ts script confirming data exists but API returns 401 Unauthorized without proper authentication. Created test-production-auth.html for production testing. Solution: Ensure users are logged in via Replit Auth to access restaurant data.
