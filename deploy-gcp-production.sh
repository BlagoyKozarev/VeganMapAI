...
KEY_FILE="attached_assets/centered-inn-460216-r9-0f1b22fc9460.json"   # Service account key
...
gcloud projects add-iam-policy-binding "$PROJECT_ID" --member="serviceAccount:${SA_EMAIL}" --role="roles/cloudbuild.builds.editor"    || true
...
# Copy existing production server
cp ../veganmapai/server-production.js ./index.js
cp ../veganmapai/package.json ./ || echo '{"type":"module","main":"index.js"}' > package.json
...
# Copy GeoJSON data
mkdir -p geojson
cp ../veganmapai/geojson/sofia.geojson ./geojson/
...
curl -s -X POST "${API_URL}/api/recommend" \
  ...
  -d '{"lat":42.6977,"lng":23.3219,"radius":3000,"minScore":8.0,"file":"'"${CDN_GEOJSON}/sofia.geojson"'"}' | head -c 300; echo
